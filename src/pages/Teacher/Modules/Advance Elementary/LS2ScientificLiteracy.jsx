...
export default function LS2ScientificLiteracy() {
    ...
    const handleUpload = async () => {
        if (pdfFile) {
          ...
          try {
            const response = await axios.post('http://localhost:8000/api/ls2scientificliteracy/upload', formData);
      ...
      const fetchPdfContents = async (id, original_name) => {
        try {
          const response = await axios.get(`http://localhost:8000/api/ls2scientificliteracy/${id}`, {
      ...
      useEffect(() => {
        const fetchContent = async () => {
          try {
            const res = await axios.get('http://localhost:8000/api/ls2scientificliteracy');
  ...
  return (
    ...
    <div className='flex gap-24'>
        <div>
            <h1 className=' font-bold text-xl'>Upload for LS2 Communication Skills</h1>
