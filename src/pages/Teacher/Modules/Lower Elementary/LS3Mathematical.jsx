...
export default function LS4Mathematical() {
    ...
    const handleUpload = async () => {
        if (pdfFile) {
          ...
          try {
            const response = await axios.post('http://localhost:8000/api/ls3mathematical/upload', formData);
      ...
      const handleDelete = async (pdf) => {
        try {
          const res = await axios.delete(`http://127.0.0.1:8000/api/ls3mathematical/${pdf.id}`); // Update the URL as needed
          // if(res.status == 200){
