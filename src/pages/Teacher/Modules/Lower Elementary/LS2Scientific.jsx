...
export default function LS2Scientific() {
    ...
    const handleUpload = async () => {
        if (pdfFile) {
          ...
          try {
            const response = await axios.post('http://localhost:8000/api/ls2scientific/upload', formData);
      ...
      const fetchPdfContents = async (id, original_name) => {
        try {
          const response = await axios.get(`http://localhost:8000/api/ls2scientific/${id}`, {
      ...
      useEffect(() => {
        const fetchContent = async () => {
          try {
            const res = await axios.get('http://localhost:8000/api/ls2scientific');
      ...
      const handleDelete = async (pdf) => {
        try {
          const res = await axios.delete(`http://127.0.0.1:8000/api/ls2scientific/${pdf.id}`); // Update the URL as needed
          // if(res.status == 200){
