...
export default function LS1ComSkills() {
    ...
    const handleUpload = async () => {
        if (pdfFile) {
          ...
          try {
            const response = await axios.post('http://localhost:8000/api/ls1commskills/upload', formData);
      ...
      const fetchPdfContents = async (id, original_name) => {
        try {
          const response = await axios.get(`http://localhost:8000/api/ls1commskills/${id}`, {
      ...
      useEffect(() => {
        const fetchContent = async () => {
          try {
            const res = await axios.get('http://localhost:8000/api/ls1commskills');
      ...
      const handleDelete = async (pdf) => {
        try {
          const res = await axios.delete(`http://127.0.0.1:8000/api/ls1commskills/${pdf.id}`); // Update the URL as needed
          // if(res.status == 200){
