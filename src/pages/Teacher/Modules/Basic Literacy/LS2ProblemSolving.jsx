...
export default function LS2ProblemSolving() {
    ...
    const handleUpload = async () => {
        if (pdfFile) {
          ...
          try {
            const response = await axios.post('http://localhost:8000/api/ls2problemsolving/upload', formData);
      ...
      const fetchPdfContents = async (id, original_name) => {
        try {
          const response = await axios.get(`http://localhost:8000/api/ls2problemsolving/${id}`, {
      ...
      useEffect(() => {
        const fetchContent = async () => {
          try {
            const res = await axios.get('http://localhost:8000/api/ls2problemsolving');
      ...
      const handleDelete = async (pdf) => {
        try {
          const res = await axios.delete(`http://127.0.0.1:8000/api/ls2problemsolving/${pdf.id}`); // Update the URL as needed
          // if(res.status == 200){
  ...
  return (
    ...
    <div className='flex gap-24'>
        <div>
            <h1 className=' font-bold text-xl'>Upload for LS2 Problem Solving Skills and Critical Thinking</h1>
