...
export default function RegularTeacherFilipinoModule() {
    ...
    const handleUpload = async () => {
        if (pdfFile) {
          ...
          try {
            const response = await axios.post('http://localhost:8000/api/filipino-module/upload', formData);
      ...
      const handleDelete = async (pdf) => {
        try {
          const res = await axios.delete(`http://127.0.0.1:8000/api/filipino-module/${pdf.id}`); // Update the URL as needed
          // if(res.status == 200){
  ...
  return (
    ...
    <div className='flex gap-24'>
        <div>
            <h1 className=' font-bold text-xl'>Upload for Filpino modules</h1>
