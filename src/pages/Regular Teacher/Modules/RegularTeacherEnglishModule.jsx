...
export default function RegularTeacherEnglishModule() {
    ...
    const handleUpload = async () => {
        if (pdfFile) {
          ...
          try {
            const response = await axios.post('http://localhost:8000/api/english-module/upload', formData);
      ...
      const fetchPdfContents = async (id, original_name) => {
        try {
          const response = await axios.get(`http://localhost:8000/api/english-module/${id}`, {
      ...
      useEffect(() => {
        const fetchContent = async () => {
          try {
            const res = await axios.get('http://localhost:8000/api/english-module');
      ...
      const handleDelete = async (pdf) => {
        try {
          const res = await axios.delete(`http://127.0.0.1:8000/api/english-module/${pdf.id}`); // Update the URL as needed
          // if(res.status == 200){
  ...
  return (
    ...
    <div className='flex gap-24'>
        <div>
            <h1 className=' font-bold text-xl'>Upload for English modules</h1>
